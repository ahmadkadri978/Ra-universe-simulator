...
export function CinematicCameraRig() {
  ...
  const definition = getUniverseScale(activeScaleId);
  ...
  const goalPosition = useMemo(
    () => new Vector3(
      ...
      definition.anchor[2] + definition.cameraOffset[2],
    ),
    [definition],
  );
  ...
  useFrame((_, delta) => {
    ...
    if (traveling) {
      const alpha = 1 - Math.exp(-delta * 2.25);
  ...
  return (
    <OrbitControls
      ...
      dampingFactor={0.055}
      enablePan={false}
      minDistance={definition.orbitMinDistance}
      maxDistance={definition.orbitMaxDistance}
      rotateSpeed={0.52}
      zoomSpeed={0.68}
      target={definition.anchor as unknown as [number, number, number]}
    />
  );
}
